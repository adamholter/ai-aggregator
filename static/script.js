...
const FILTERABLE_SECTIONS = {
    ...
    'models-dev': { sectionId: 'models-dev', category: 'models-dev', limit: 60, getItems: () => rawData.modelsDev || [] },
...
function stripModelQualifiers(value) {
    if (!value) return '';
    return normalizeModelName(String(value).replace(/\([^)]*\)|\[[^\]]*\]/g, ' '));
}
...
function buildOpenRouterIndex(models) {
    ...
    models.forEach(model => {
        ...
        const normalizedBaseName = normalizeModelName(model.base_name);
        const normalizedId = normalizeModelName(model.id);
        const normalizedSlug = normalizeModelName(model.slug);
        const variants = new Set([
            normalizedName,
            normalizedBaseName,
            normalizedId,
            normalizedSlug,
            stripModelQualifiers(model.name),
            stripModelQualifiers(model.base_name),
            removeMatchTokens(normalizedName),
            removeMatchTokens(normalizedBaseName),
            simplifyWithVendor(normalizedName, model.vendor),
            simplifyWithVendor(normalizedBaseName, model.vendor),
            simplifyWithVendor(normalizedId, model.vendor),
            simplifyWithVendor(normalizedSlug, model.vendor)
        ]);
...
function findOpenRouterMatch(modelName, providerHint = '') {
    ...
    const withoutQualifiers = stripModelQualifiers(modelName);
    const candidates = [
        target,
        simplified,
        withoutQualifiers,
        removeMatchTokens(withoutQualifiers),
        simplifyWithVendor(target, providerHint),
        simplifyWithVendor(simplified, providerHint),
        simplifyWithVendor(withoutQualifiers, providerHint)
    ].filter(Boolean);

    for (const candidate of candidates) {
        const matches = openRouterIndex.get(candidate);
        if (matches && matches.length > 0) {
            const match = selectPreferredMatch(matches, providerHint, withoutQualifiers || target);
        ...
        keys.forEach(key => {
            ...
            if (score > bestScore && score >= 0.68) {
                ...
                if (match && match.length > 0) {
                    bestScore = score;
                    bestMatch = selectPreferredMatch(match, providerHint, withoutQualifiers || target);
                }
            }
        });
    });
...
function simplifyWithVendor(value, vendor) {
    ...
    return value
        .replace(normalizedVendor, '')
        .replace(/^[\s\-/]+/, '')
        .replace(/\s+/g, ' ')
        .trim();
}

function selectPreferredMatch(matches, providerHint, desiredName = '') {
    if (!matches || matches.length === 0) return null;
    const normalizedHint = normalizeModelName(providerHint);
    const desired = stripModelQualifiers(desiredName);
    return [...matches].sort((left, right) => {
        const score = model => {
            const rawBase = simplifyWithVendor(normalizeModelName(model.base_name || model.name || model.id), model.vendor);
            const base = simplifyWithVendor(stripModelQualifiers(model.base_name || model.name || model.id), model.vendor);
            const identityScore = base === desired ? 100 : similarity(desired, base) * 10;
            const unqualifiedScore = rawBase === desired ? 25 : 0;
            const providerScore = normalizedHint && normalizeModelName(model.vendor || '').includes(normalizedHint) ? 1 : 0;
            return identityScore + unqualifiedScore + providerScore;
        };
...
function ensureExperimentalNavButtons() {
    ...
    ensureButton('models-dev', 'models.dev');
    ensureButton('personal-index', 'Personal Index');
...
function requireUsefulItems(items, sourceLabel, requiredFields = []) {
    if (!Array.isArray(items) || items.length === 0) {
        throw new Error(`${sourceLabel} returned no usable records`);
    }

    const usefulItems = requiredFields.length
        ? items.filter(item => item && requiredFields.some(field => {
    ...
    if (usefulItems.length === 0) {
        throw new Error(`${sourceLabel} returned records without useful identifying data`);
    }
    return items;
}
...
async function fetchModelsDevData(forceRefresh = false) {
    if (cachedData.modelsDev && !forceRefresh) {
        return cachedData.modelsDev;
    }
    const params = new URLSearchParams({ limit: String(MODELS_DEV_UI_LIMIT) });
    if (forceRefresh) params.set('cache_bust', 'true');
    const data = await makeAPICall(`/api/models-dev?${params.toString()}`, null);
    const rows = requireUsefulItems(data, 'models.dev', ['name', 'model_id', 'provider']);
    cachedData.modelsDev = rows;
    rawData.modelsDev = rows;
    return rows;
}

async function loadModelsDevData(forceRefresh = false) {
    const loadingElement = document.getElementById('models-dev-loading');
    const errorElement = document.getElementById('models-dev-error');
    const dataElement = document.getElementById('models-dev-data');

    if (!loadingElement || !errorElement || !dataElement) return;
    ...
    try {
        ...
        const rows = await fetchModelsDevData(forceRefresh);
        populateModelsDevProviderFilter(rows);
        filterModelsDevData();
        loadingElement.style.display = 'none';
    } catch (error) {
        console.error('Failed to load models.dev data:', error);
        loadingElement.style.display = 'none';
        errorElement.textContent = `Failed to load models.dev data: ${error.message}`;
        errorElement.style.display = 'block';
    }
}

function populateModelsDevProviderFilter(models) {
    const providerInput = document.getElementById('models-dev-provider');
    const providerOptions = document.getElementById('models-dev-provider-options');
    if (!providerInput || !providerOptions || !Array.isArray(models)) return;

    const providers = Array.from(new Set(models.map(model => model.provider).filter(Boolean))).sort();
    const currentValue = providerInput.value;
    providerOptions.innerHTML = providers.map(provider => `<option value="${escapeHtml(provider)}"></option>`).join('');
    if (currentValue && !providers.includes(currentValue)) providerInput.value = '';
}

function filterModelsDevData() {
    if (!rawData.modelsDev) return;

    const loadingElement = document.getElementById('models-dev-loading');
    if (loadingElement) loadingElement.style.display = 'none';

    const searchInput = document.getElementById('models-dev-search');
    const sortSelect = document.getElementById('models-dev-sort');
    const providerInput = document.getElementById('models-dev-provider');

    const searchTerm = ((searchInput && searchInput.value) || '').toLowerCase();
    const sortBy = (sortSelect && sortSelect.value) || 'updated';
    const providerFilter = (providerInput && providerInput.value) || '';

    let filtered = rawData.modelsDev.filter(model => {
    ...
    filtered = sortModelsDevData(filtered, sortBy);
    const displayModels = getFilteredItems('models-dev', filtered);
    displayModelsDevData(displayModels);

    const resultsInfo = document.getElementById('models-dev-results-info');
    if (resultsInfo) {
        resultsInfo.textContent = `Showing ${displayModels.length} of ${rawData.modelsDev.length} models`;
    }
}

function sortModelsDevData(models, sortBy) {
    const sorted = [...models];
    switch (sortBy) {
        case 'name':
        ...
        case 'input-cost':
            return sorted.sort((a, b) => {
                const aCost = a.input_cost == null ? Infinity : Number(a.input_cost);
                const bCost = b.input_cost == null ? Infinity : Number(b.input_cost);
                return aCost - bCost;
            });
        case 'updated':
        default:
            return sorted.sort((a, b) => (b.last_updated || '').localeCompare(a.last_updated || ''));
    }
}

function displayModelsDevData(models) {
    const container = document.getElementById('models-dev-data');
    if (!container) return;

    container.innerHTML = '';
    recordDisplayedItems('models-dev', models);
    models.forEach(model => container.appendChild(createModelsDevCard(model)));
}
...
async function fetchHypeData(forceRefresh = false) {
    ...
    const url = forceRefresh ? '/api/hype-live?cache_bust=true' : '/api/hype-live';
    const data = await makeAPICall(url, null);
...
function displayHypeItems(payload) {
    ...
    if (resultsInfo) {
        ...
        const sourceCounts = displayItems.reduce((counts, item) => {
            const label = formatHypeSource(item?.source);
        ...
        const sourceSummary = Object.entries(sourceCounts)
            ...
            .join(', ');
        if (sourceSummary) summary.push(sourceSummary);
...
function createBlogCard(post) {
    ...
    const imageMarkup = featuredImage
        ? `<a class="card-media" href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer" aria-label="Read ${escapeHtml(titleText)}">
                <img src="${escapeHtml(featuredImage)}" alt="" loading="lazy" decoding="async">
           </a>`
        : '';
...
function displayMonitorItems(items) {
    ...
    const featured = displayItems.filter(item => item.is_featured).slice(0, 3);
    const recent = displayItems.filter(item => !featured.includes(item));
    if (featured.length) {
        const featuredSection = document.createElement('section');
        featuredSection.className = 'monitor-featured-section';
        featuredSection.innerHTML = '<div class="monitor-section-heading"><div><span class="monitor-kicker">Big picture</span><h3>Stories that matter most</h3></div><p>Ranked by impact, evidence, and staying power—not just timestamp.</p></div>';
        const grid = document.createElement('div');
        grid.className = 'monitor-featured-grid';
        featured.forEach(item => grid.appendChild(createMonitorCard(item)));
        featuredSection.appendChild(grid);
        container.appendChild(featuredSection);
    }
    if (recent.length) {
        const recentSection = document.createElement('section');
        recentSection.className = 'monitor-recent-section';
        recentSection.innerHTML = '<div class="monitor-section-heading monitor-recent-heading"><div><span class="monitor-kicker">Latest updates</span><h3>Recent monitoring</h3></div><p>Everything else, newest first.</p></div>';
        const list = document.createElement('div');
        list.className = 'monitor-recent-list';
        recent.forEach(item => list.appendChild(createMonitorCard(item)));
...
function createMonitorCard(item) {
    const card = document.createElement('div');
    card.className = `model-card latest-card${item.is_featured ? ' monitor-featured-card' : ''}`;
    ...
    const sourceLabel = item.source_label || 'Monitor Feed';
    const metaParts = [`<span class="meta-item">${escapeHtml(sourceLabel)}</span>`];
    if (item.is_featured && item.importance_score != null) {
        metaParts.push(`<span class="meta-item monitor-importance">Importance ${Math.round(Number(item.importance_score))}/100</span>`);
    }
    if (relative) {
...
function createModelsDevCard(model) {
    const card = document.createElement('div');
    card.className = 'model-card';
    card.dataset.source = 'models-dev';
    card.dataset.itemKey = model.id || model.model_id || model.name;

    const title = escapeHtml(model.name || model.model_id || 'Unknown Model');
    const provider = escapeHtml(model.provider || 'models.dev');
    const contextLength = model.context_display || formatContextLength(model.context_length);
    const outputLimit = model.output_limit_display || formatContextLength(model.output_limit);
    const inputCost = model.input_cost_display || (model.input_cost == null ? 'Not provided' : `$${Number(model.input_cost).toFixed(2)}`);
    const outputCost = model.output_cost_display || (model.output_cost == null ? 'Not provided' : `$${Number(model.output_cost).toFixed(2)}`);
    const modalities = [
        Array.isArray(model.input_modalities) && model.input_modalities.length ? `In: ${model.input_modalities.join(', ')}` : '',
        Array.isArray(model.output_modalities) && model.output_modalities.length ? `Out: ${model.output_modalities.join(', ')}` : ''
    ...
    ].filter(Boolean).join(' · ') || 'Standard completion';
    const updated = model.last_updated ? escapeHtml(model.last_updated) : '';
    const modelId = model.model_id ? escapeHtml(model.model_id) : '';
    ...
    card.innerHTML = `
        ...
        <div class="model-stats">
            <div class="stat-item">
                <span class="stat-label">Context Length</span>
                <span class="stat-value">${escapeHtml(contextLength || 'Not provided')}</span>
            </div>
            <div class="stat-item">
                <span class="stat-label">Output Limit</span>
                <span class="stat-value">${escapeHtml(outputLimit || 'Not provided')}</span>
            </div>
        </div>
        <div class="pricing">
            <h4>Pricing</h4>
            <div class="evaluation-item"><span>Input</span><span>${escapeHtml(inputCost)}</span></div>
            <div class="evaluation-item"><span>Output</span><span>${escapeHtml(outputCost)}</span></div>
        </div>
        <div class="model-meta">
            ${modelId ? `<div><strong>ID:</strong> ${modelId}</div>` : ''}
            <div><strong>Modalities:</strong> ${escapeHtml(modalities)}</div>
            <div><strong>Capabilities:</strong> ${escapeHtml(capabilities)}</div>
            ${updated ? `<div><strong>Updated:</strong> ${updated}</div>` : ''}
        </div>
        <div class="links-block">
            ${model.provider_doc ? `<a class="external-link" href="${escapeHtml(model.provider_doc)}" target="_blank" rel="noopener">Provider Docs ↗</a>` : ''}
            ${model.provider_api ? `<a class="external-link" href="${escapeHtml(model.provider_api)}" target="_blank" rel="noopener">Provider API ↗</a>` : ''}
        </div>
    `;
    attachPinButton(card, 'models-dev', model);
    return card;
}
...
function setupModelsDevControls() {
    const refresh = document.getElementById('models-dev-refresh');
    if (refresh && refresh.dataset.listenerAttached !== 'true') {
        refresh.addEventListener('click', () => {
            cachedData.modelsDev = null;
            rawData.modelsDev = null;
            loadModelsDevData(true);
        });
        refresh.dataset.listenerAttached = 'true';
    }
}
...
function configureOpenRouterModalTab(modalContent, model, type) {
    ...
    tabButton.textContent = 'OpenRouter Data';
    tabButton.dataset.lookupStatus = 'loading';

    const section = createSection('OpenRouter Catalogue');
    section.setAttribute('aria-live', 'polite');
    tabContent.appendChild(section);
    populateOpenRouterComparison(section, model).then(result => {
        updateOpenRouterTabLookupStatus(modalContent, result && result.status);
    });
}

function updateOpenRouterTabLookupStatus(modalContent, status) {
    const tabButton = modalContent && modalContent.querySelector('.modal-tab[data-tab="openrouter"]');
    if (!tabButton) return;
    tabButton.dataset.lookupStatus = status || 'error';
    if (status === 'matched') {
        tabButton.textContent = 'OpenRouter Data ✓';
        tabButton.title = 'OpenRouter data ready';
    } else if (status === 'not-found') {
        tabButton.textContent = 'OpenRouter Data —';
        tabButton.title = 'No matching OpenRouter model found';
    } else {
        tabButton.textContent = 'OpenRouter Data !';
        tabButton.title = 'OpenRouter lookup failed';
    }
}
...
async function populateOpenRouterComparison(section, model) {
    ...
    try {
        const catalogue = await ensureOpenRouterDataLoaded();
        if (!Array.isArray(catalogue) || !catalogue.length) {
            throw new Error('OpenRouter returned an empty catalogue.');
        }
        Array.from(section.children).forEach(child => {
            if (child !== heading) {
                section.removeChild(child);
        ...
        const localMatch = findOpenRouterMatch(model.name, providerName);
        if (localMatch) {
            section.appendChild(renderOpenRouterSummary(localMatch));
            return { status: 'matched', match: localMatch };
        }
        ...
        if (match && match.metadata) {
            ...
            return { status: 'matched', match: match.metadata };
        } else {
        ...
        const state = createEmptyState(error && error.message
            ? `OpenRouter lookup failed: ${error.message}`
            : 'OpenRouter lookup failed.');
        const retry = document.createElement('button');
        retry.type = 'button';
        retry.className = 'chart-btn secondary openrouter-retry-btn';
        retry.textContent = 'Retry lookup';
        retry.addEventListener('click', async () => {
            const modalContent = section.closest('.modal-content');
            const tabButton = modalContent && modalContent.querySelector('.modal-tab[data-tab="openrouter"]');
            if (tabButton) {
                tabButton.textContent = 'OpenRouter Data';
                tabButton.dataset.lookupStatus = 'loading';
            }
            const result = await populateOpenRouterComparison(section, model);
            updateOpenRouterTabLookupStatus(modalContent, result && result.status);
        });
...
function getComparableBenchmarkEntries(model) {
    return Object.entries((model && model.evaluations) || {})
...
function addToComparison(model) {
    ...
    if (!getComparableBenchmarkEntries(model).length) {
        showToast('This model does not have comparable benchmark data', 'warning');
        return;
    }

    // More than five radar polygons becomes hard to read and compare.
    if (chartComparisonModels.length >= 5) {
        showToast('Maximum 5 models for comparison', 'warning');
        return;
    }

    chartComparisonModels.push(model);
    showToast(`Added ${model.name || modelId} to comparison (${chartComparisonModels.length}/5)`, 'info');
    updateCompareButtonStates();
}

function attachCompareButton(card, model) {
    if (!card || !model) return;
    if (!getComparableBenchmarkEntries(model).length) return;
    const modelId = model.id || model.name || model.model;
    if (!modelId) return;
...
function openChartModal() {
    ...
    // Render the same benchmark shape used by individual model details.
    renderChart();
...
function formatComparisonBenchmarkName(key) {
    if (typeof window.getEvalDisplayName === 'function') return window.getEvalDisplayName(key);
    return key.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
}

function normalizeComparisonBenchmark(value) {
    const number = Number(value);
    if (!Number.isFinite(number)) return null;
    return Math.max(0, Math.min(100, number > 1 ? number : number * 100));
}

function buildComparisonAxisDomain(values) {
    const normalizedValues = values.map(normalizeComparisonBenchmark).filter(Number.isFinite);
    const min = Math.min(...normalizedValues);
    const max = Math.max(...normalizedValues);
    const range = max - min;
    // Small real spreads should remain visible without turning every axis into
    // a best-vs-worst extreme. Wider spreads receive slightly more contrast.
    const amplitude = Math.min(22, Math.max(12, 12 + (range * 1.25)));
    return { lower: min, upper: max, amplitude };
}

function scaleComparisonBenchmarkToAxis(value, domain) {
    const normalized = normalizeComparisonBenchmark(value);
    if (!Number.isFinite(normalized) || !domain || domain.upper === domain.lower) return 50;
    const midpoint = (domain.lower + domain.upper) / 2;
    const halfRange = (domain.upper - domain.lower) / 2;
    return 50 + (((normalized - midpoint) / halfRange) * domain.amplitude);
}

function formatComparisonBenchmarkValue(value) {
    const number = Number(value);
    if (!Number.isFinite(number)) return 'N/A';
    return number > 1 ? number.toFixed(1) : number.toFixed(3);
}

function wrapComparisonRadarLabel(label) {
...
function renderComparisonScoreTable(container, benchmarkKeys) {
    const tableWrap = document.getElementById('chart-score-table');
    if (!tableWrap) return;
    const table = document.createElement('table');
    table.className = 'comparison-score-table';
    const head = document.createElement('thead');
    const headRow = document.createElement('tr');
    headRow.appendChild(Object.assign(document.createElement('th'), { textContent: 'Model' }));
    benchmarkKeys.forEach(key => headRow.appendChild(Object.assign(document.createElement('th'), { textContent: formatComparisonBenchmarkName(key) })));
    head.appendChild(headRow);
    table.appendChild(head);
    const body = document.createElement('tbody');
    chartComparisonModels.forEach(model => {
        const row = document.createElement('tr');
        row.appendChild(Object.assign(document.createElement('th'), { textContent: model.name || model.id || 'Model' }));
        benchmarkKeys.forEach(key => row.appendChild(Object.assign(document.createElement('td'), { textContent: formatComparisonBenchmarkValue(model.evaluations[key]) })));
...
/** Render a multi-model benchmark radar locally from the selected card data. */
function renderChart() {
    const container = document.getElementById('chart-container');
    if (!container) return;
    const benchmarkKeys = getComparableBenchmarkEntries(chartComparisonModels[0])
        .map(([key]) => key)
        .filter(key => chartComparisonModels.every(model => Number.isFinite(Number(model.evaluations && model.evaluations[key]))))
        .sort((left, right) => {
            const leftIndex = left.startsWith('artificial_analysis');
            const rightIndex = right.startsWith('artificial_analysis');
            if (leftIndex !== rightIndex) return leftIndex ? -1 : 1;
            return formatComparisonBenchmarkName(left).localeCompare(formatComparisonBenchmarkName(right));
        });

    if (benchmarkKeys.length < 3 || typeof Chart !== 'function') {
        container.innerHTML = '<div class="comparison-chart-empty">These models do not share enough benchmark data for a radar comparison.</div>';
        const tableWrap = document.getElementById('chart-score-table');
    ...
    canvas.setAttribute('aria-label', `Radar chart comparing ${chartComparisonModels.length} models across ${benchmarkKeys.length} shared benchmarks, with each axis scaled to the selected models' score range`);
    container.appendChild(canvas);
    const styles = getComputedStyle(document.documentElement);
    const ink = styles.getPropertyValue('--text-color').trim() || '#111827';
    const border = styles.getPropertyValue('--border-color').trim() || '#d1d5db';
    const exactValues = chartComparisonModels.map(model => benchmarkKeys.map(key => model.evaluations[key]));
    const axisDomains = benchmarkKeys.map((key, benchmarkIndex) =>
        buildComparisonAxisDomain(exactValues.map(modelValues => modelValues[benchmarkIndex]))
    );
    const formatAxisDomain = (domain, benchmarkIndex) => {
        const usesFraction = exactValues.every(modelValues => Number(modelValues[benchmarkIndex]) <= 1);
        const divisor = usesFraction ? 100 : 1;
        return `${formatComparisonBenchmarkValue(domain.lower / divisor)}–${formatComparisonBenchmarkValue(domain.upper / divisor)}`;
    };
    comparisonRadarChart = new Chart(canvas, {
        type: 'radar',
        data: {
            labels: benchmarkKeys.map(key => wrapComparisonRadarLabel(formatComparisonBenchmarkName(key))),
            datasets: chartComparisonModels.map((model, index) => {
                const color = COMPARISON_RADAR_COLORS[index];
                const fullLabel = model.name || model.id || `Model ${index + 1}`;
                return {
                    label: fullLabel.length > 42 ? `${fullLabel.slice(0, 39)}…` : fullLabel,
                    fullLabel,
                    data: exactValues[index].map((value, benchmarkIndex) =>
                        scaleComparisonBenchmarkToAxis(value, axisDomains[benchmarkIndex])
                    ),
                    borderColor: color,
        ...
        options: {
            ...
            scales: { r: {
                ...
                pointLabels: { color: ink, font: { size: 11, weight: '500' }, padding: 8 }
            } },
            plugins: {
                legend: { position: 'bottom', labels: { color: ink, usePointStyle: true, boxWidth: 8, padding: 16 } },
                tooltip: { callbacks: {
                    title: items => formatComparisonBenchmarkName(benchmarkKeys[items[0].dataIndex]),
                    label: context => `${context.dataset.fullLabel}: ${formatComparisonBenchmarkValue(exactValues[context.datasetIndex][context.dataIndex])}`,
                    afterLabel: context => `Axis range: ${formatAxisDomain(axisDomains[context.dataIndex], context.dataIndex)}`
                } }
            }
        }
    });
    renderComparisonScoreTable(container, benchmarkKeys);
}
...
window.addEventListener('message', (event) => {
    if (event.origin !== window.location.origin || event.data?.type !== 'personal-index:resize') return;
    ...
    const height = Math.max(720, Math.min(16000, Math.ceil(requestedHeight)));
    frame.style.height = `${height}px`;
    const container = frame.closest('.personal-index-iframe-container');
