...
from backend.app.services.models_dev import load_models_dev_catalog
...
HYPE_SUPABASE_SOURCES = (os.environ.get('HYPE_SUPABASE_SOURCES') or 'github,huggingface,reddit,replicate,hackernews').strip()
...
FETCH_DATA_CATEGORY_CONFIG = {
    ...
    'models-dev': {
        'label': 'models.dev Catalogue',
        'endpoint': '/api/models-dev',
        'params': None,
        'cache_keys': lambda: [get_cache_key('models_dev')],
        'extract': lambda payload: payload if isinstance(payload, list) else (payload or []),
        'limit': None,
        'source': 'models.dev'
    },
...
AGENT_EXP_CORE_TABS = [
    ...
    },
    {
        'id': 'models-dev',
        'label': 'models.dev Catalogue',
        'description': 'Provider-normalized model specs covering limits, modalities, capabilities, and costs.',
        'guidance': 'Use for cross-provider capability checks, SDK/provider metadata, limits, and caching/tool-use support.'
    },
    {
        'id': 'text-to-image',
...
def infer_item_provider(category_id, item):
    ...
    if category_id == 'models-dev':
        return item.get('provider') or ''
    if category_id == 'fal':
...
def infer_item_metrics(category_id, item):
    ...
    if category_id == 'llms':
    ...
    elif category_id == 'models-dev':
        if item.get('context_length') is not None:
            metrics['context'] = item['context_length']
        if item.get('output_limit') is not None:
            metrics['output_limit'] = item['output_limit']
        if item.get('input_cost') is not None:
            metrics['input_cost'] = item['input_cost']
        if item.get('output_cost') is not None:
            metrics['output_cost'] = item['output_cost']
        capabilities = []
        ...
        if capabilities:
            metrics['capabilities'] = ', '.join(capabilities)
...
def _normalize_blog_post(entry):
    ...
    if not featured_image:
        featured_image = entry.get('jetpack_featured_media_url') or ''
...
@app.route('/api/models-dev', methods=['GET'])
def get_models_dev():
    """Fetch and cache the normalized models.dev catalogue."""
    force_refresh = request.args.get('cache_bust', 'false').lower() == 'true'
    limit = _coerce_int(request.args.get('limit'))
    cache_key = get_cache_key('models_dev')

    if not force_refresh and cache_key in cache and is_cache_valid(cache[cache_key]['timestamp']):
        rows = cache[cache_key]['data']
        return jsonify(rows[:limit] if limit else rows)

    try:
        rows = load_models_dev_catalog(force_refresh=force_refresh)
        cache[cache_key] = build_cache_entry(rows)
        _record_provider_check('models.dev')
        response_rows = rows[:limit] if limit else rows
        ...
        return jsonify({'error': f'Failed to fetch models.dev catalogue: {exc}'}), 500
    except Exception as exc:
        return jsonify({'error': f'Error processing models.dev catalogue: {exc}'}), 500


def fetch_hype_feed_payload(limit=None, window_days=None):
    """Fetch a source-balanced hype feed with optional window and limit settings."""
    ...
    source_tokens = [segment.strip().lower() for segment in HYPE_SUPABASE_SOURCES.split(',') if segment.strip()]
    if not source_tokens:
        source_tokens = ['github', 'huggingface', 'reddit', 'replicate', 'hackernews']
    ...
    source_aliases = {
        ...
        'reddit': ('reddit',),
        'replicate': ('replicate',),
        'hackernews': ('hackernews', 'hacker-news', 'hacker_news', 'hn'),
    }
    requested_sources = []
    for token in source_tokens:
        canonical = next((name for name, aliases in source_aliases.items() if token in aliases), token)
        if canonical not in requested_sources:
            requested_sources.append(canonical)
    # These are core Hype sources. Merge them even when an older deployment has
    # an environment override, otherwise that stale setting silently hides data.
    for core_source in ('github', 'reddit', 'replicate', 'hackernews'):
        if core_source not in requested_sources:
            requested_sources.append(core_source)
    ...
    def fetch_hacker_news(item_limit):
        """Use HN's public search API when the warehouse has no HN rows yet."""
        response = requests.get(
            ...
            params={
                ...
                'hitsPerPage': str(item_limit),
            },
            timeout=HYPE_SUPABASE_TIMEOUT_SECONDS,
        )
        response.raise_for_status()
        hits = response.json().get('hits', [])
        return [_normalize_hype_item({
            'name': hit.get('title') or hit.get('story_title'),
            'url': hit.get('url') or f"https://news.ycombinator.com/item?id={hit.get('objectID')}",
            'stars': hit.get('points') or 0,
            'username': hit.get('author'),
            'source': 'hackernews',
            'description': hit.get('story_text'),
            'created_at': hit.get('created_at'),
            'inserted_at': hit.get('created_at'),
        }) for hit in hits if hit.get('title') or hit.get('story_title')]

    try:
        per_source_limit = max(1, (limit + len(requested_sources) - 1) // len(requested_sources))
        items = []
        source_counts = {}
        for source_name in requested_sources:
            ...
            params = {
                ...
                'source': f"in.({','.join(aliases)})",
                # inserted_at represents when the crawler observed the item.
                # Filtering on repository created_at also excluded older projects
                # that had only just begun trending.
                'inserted_at': f'gt.{since_iso}',
            }
            source_items = fetch_feed(params)
            if not source_items and window_days:
                params.pop('inserted_at', None)
                source_items = fetch_feed(params)
            if source_name == 'hackernews' and not source_items:
                try:
                    source_items = fetch_hacker_news(per_source_limit)
        ...
        deduped = {}
        for item in items:
            identity = item.get('url') or f"{item.get('source')}:{item.get('name')}"
            if identity and identity not in deduped:
                deduped[identity] = item
        items = sorted(
            deduped.values(),
            key=lambda item: (item.get('stars') or 0, item.get('inserted_at') or item.get('created_at') or ''),
            reverse=True,
        )[:limit]
    except requests.exceptions.HTTPError as exc:
    ...
    return {
        ...
        'meta': {
            ...
            'sources': ','.join(requested_sources),
            'source_counts': source_counts,
            'retrieval': 'balanced-per-source'
        }
    }
...
def _fetch_sheet_rows_via_api(range_name):
    ...
    # Keep the whole Monitor endpoint inside the production worker deadline even
    # when Google is slow and the CSV fallback must also be attempted.
...
def _fetch_database_monitor_entries():
    try:
        from backend.app import db as _db
        from backend.app.monitor_ranking import effective_importance
        rows = _db.query_all_http("SELECT id,source,source_label,title,excerpt,url,author,published_at,observed_at,thread_context,tags_json,importance_score,importance_reason,story_key,evidence_status FROM monitor_entries ORDER BY observed_at DESC LIMIT 3000")
    except Exception as exc:
        print(f"WARNING: Failed to fetch database monitor entries: {exc}")
        return []
    entries = []
    for row in rows:
        dt = _coerce_timestamp_utc(row.get('published_at') or row.get('observed_at'))
        if not dt or not row.get('url'): continue
        try: tags = json.loads(row.get('tags_json') or '[]')
        except (TypeError, ValueError): tags = []
        importance = float(row.get('importance_score') or 40)
        entries.append({'id': row.get('id'), 'title': row.get('title') or 'Monitor Update', 'source': row.get('source') or 'monitor',
            'source_label': row.get('source_label') or 'Monitor Feed', 'excerpt': _truncate_text((row.get('thread_context') or row.get('excerpt') or '').strip(), 280),
            'timestamp_dt': dt, 'timestamp': dt.replace(microsecond=0).isoformat().replace('+00:00', 'Z'), 'url': row.get('url'),
            'badge': row.get('author') or row.get('source_label') or 'Monitor', 'tags': tags if isinstance(tags, list) else [],
            'importance_score': importance, 'importance_reason': row.get('importance_reason') or '', 'story_key': row.get('story_key') or row.get('id'),
            'evidence_status': row.get('evidence_status') or '', 'effective_importance': effective_importance(importance, row.get('published_at') or row.get('observed_at'))})
    # Feature one strong representative per story. Importance decays with a
    # score-dependent half-life; high-impact launches persist while minor news
    # naturally falls back into the chronological feed.
    featured, seen_stories = [], set()
    for entry in sorted(entries, key=lambda item: item.get('effective_importance', 0), reverse=True):
        key = entry.get('story_key') or entry.get('id')
        if key in seen_stories or entry.get('importance_score', 0) < 70:
            continue
        entry['is_featured'] = True
        featured.append(entry); seen_stories.add(key)
        if len(featured) == 3: break
    featured_ids = {entry['id'] for entry in featured}
    recent = sorted((entry for entry in entries if entry['id'] not in featured_ids), key=lambda item: item['timestamp_dt'], reverse=True)
    entries = featured + recent
    return entries
...
def load_monitor_feed(force_refresh=False, limit=None, sanitize=False):
    ...
    if (
        ...
        # Turso is now the canonical Monitor store. The old Sheets are a
        # migration fallback only; fetching them first made every uncached
        # production request wait on stale, linkless data and could exceed the
        # worker deadline.
        entries = _fetch_database_monitor_entries()
        if not entries:
